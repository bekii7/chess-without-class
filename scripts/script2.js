...
movePawn('pawn-b-1',`${move.nextMove}`,`${move.previousMove}`,'#000000' )
...
movePawn('pawn-b-8','25','17','#000000')
...
function mover(pawn,nextMove,previousMove,color){
  console.log()
...
function showmove(cellA,cellR,pawn,color){
  ...
  document.querySelector(`.${pawn}`).addEventListener("click",()=>{
    mover(pawn,move.nextMove,move.previousMove,color)
    });
}
